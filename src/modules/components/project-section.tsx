...
export const ProjectSection: React.FC = () => {
  ...
  return (
    <section
      id="projects"
      className="snap-start bg-gradient-to-t from-black to-[#232323] text-white"
    >
      <div
        id="container"
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-center text-center mb-12" data-aos="fade-up">
            <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight text-[#af76c4] mb-2">
              Projects
            </h1>
          ...
          <div className="hidden md:flex w-full max-w-6xl h-[80vh] gap-8">
            ...
            <div className="w-1/3 flex flex-col overflow-y-auto scrollbar-thin scrollbar-thumb-[#af76c4]/40 pr-2">
              {projects.map((project) => (
                <button
                  ...
                  className={`group flex items-center gap-4 p-3 rounded-xl border border-transparent hover:border-[#af76c4]/40 hover:bg-[#1e1e1e] transition-all duration-300 ${selectedProject?.id === project.id
            ...
            <div
              className={`flex-1 max-h-[60vh] bg-[#1a1a1a] rounded-2xl border border-[#af76c4]/20 p-6 transition-all duration-500 ${selectedProject ? "opacity-100 translate-x-0" : "opacity-50 translate-x-6"
                }`}
            >
              {selectedProject ? (
          ...
          {/* Mobile fallback - your existing grid layout */}
          <div className="grid grid-cols-1 md:hidden gap-10 max-w-6xl w-full px-4 mt-8">
            {projects.map((project) => (
              <div
                key={project.id}
                className="bg-[#1c1c1c] border border-transparent hover:border-[#af76c4]/40 rounded-2xl overflow-hidden shadow-lg transition-all duration-300"
              >
